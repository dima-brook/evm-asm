use crate::*;
use std::fs;

#[test]
fn evm_test() {
    println!("Small Test EVM");

    assert!(
        helpers::disassemble_evm(&helpers::bytes_from_hex("0x608040526002610100").unwrap()).is_ok()
    );
}

#[test]
fn evm_test_large() {
    println!("large test evm");

    assert!(helpers::disassemble_evm(&helpers::bytes_from_hex("").unwrap()).is_ok());
}

#[test]
fn move_test() {
    println!("move test");

    let movc = helpers::move_code_from_modfs(
        &fs::read("./data/compiled/f.mv").unwrap(),
        vec!["./data/compiled/Coin.mv"],
    );
    assert!(movc.is_ok());
    assert!(movc.unwrap().disassemble_with_mods().is_ok());
}
